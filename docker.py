  ...
  def receive(self, routing_key):
      ...
      def callback(ch, method, properties, body):
        ...
        def return_add(return_message):
            ...
            ok = system('docker run --name '+ return_message +' --net 7940 -v /var/run/docker.sock:/var/run/docker.sock -v /usr/bin/docker:/usr/bin/docker python chatbot.py --config docker_config.ini --ip '+return_message)
            logging.info(ok)
